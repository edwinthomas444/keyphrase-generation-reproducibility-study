...
class TransformerSetDecoder(nn.Module):
    ...
    def generator(self, input_dict):
        ...
        N0 = input_dict["N"] # batch_size*max_kp_num
        ...
        N = input_idx.size(0) # batch_size*max_kp_num
        ...
        B = N // N0 
        # current beam width (in multiples of bw given in config till max_beam_size
        # eg: 1, 6, 36, 40 if 6 is bw and and max_beam_size=40

        # N00 = batch_size (fixed)
        # N0 = batch_size * max_kp_num (fixed)
        # N = batch_size * max_kp_num * beam_width * (some variable)
        N00 = N0 // max_kp_num
        # print('Inside Generate:', 'N: ',N, 'N0: ',N0, 'N00: ',N00, 'B: ',B)
        ...
        # print('Decoder input for beam search: ', input_idx_.size())
        ...
        if (max_B == 1) or (B == 1):
            ...
            # as N includes batch dim, how is top-k computed correctly here?
    ...
    def hungarian_assign(self, decode_dist, target, ignore_indices, random=False):
        ...
        if random:
            ...
            # print('\n Hungarian Assign reorder cols: ', reorder_cols)
    ...
    def forward(self, src_idx, ptr_src_idx, input_mask, max_oov_num, first_mask=None, trg_idx=None, labels=None,
        ...
        # print('target idx shape: ',trg_idx.shape)
        batch_size = src.size(0)
        # print('\nFrom transformerSetDecoder batch size: ',batch_size, src.shape)
        ...
        if not self.config["generate"]:
            ...
            with T.no_grad():
                ...
                # note: below flag causes synch issues with DataParallel (Needs fix)
                ...
                for t in range(1, self.config["assign_steps"] + 1):
                    ...
                    # setting temp flag to true
                    decoder_dist, _ = self.decoder(decoder_inputs, state, src_oov, max_num_oov, control_embed, temp_fix=True)
                    # print('decoder dist shape: ',decoder_dist.shape)
                    # print('input_tokens shape: ',input_tokens.shape)
                    # print('arg max shape: ',decoder_dist.argmax(-1).shape)
                ...
                ab_reorder_index = self.hungarian_assign(decoder_dists[:, mid_idx:],
                                                         labels[:, mid_idx:, :assign_steps],
                                                         ignore_indices=[null_id, pad_id])
                # print('\n ab reorder index: ', ab_reorder_index)
            ...
            # if input_tgt.get_device()==0:
            #     # print('\n input tgt (for decoding): ', input_tgt.shape)
            #     # print('\n', input_tgt[:,10:,:])
            ...
            # print('\n Max decoder length :', self.max_decoder_len)
