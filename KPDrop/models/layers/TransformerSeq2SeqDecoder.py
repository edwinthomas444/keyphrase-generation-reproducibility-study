...
class TransformerSeq2SeqDecoder(nn.Module):
    ...
    def forward_seg(self, state, first_mask=None):
        ...
        if self.config["contextualized_control_codes"]:
            ...
            # print('\n control idxs: ', control_idx)
            control_embed = self.control_code(control_idx)
            scores = None

        return control_embed, scores

    def forward(self, tokens, state, src_oov, max_num_oov, control_embed=None, temp_fix=False):
        """
        :param torch.LongTensor tokens: batch x tgt_len，decode的词
         tokens size = b, N (is the Number of Control codes of KP units), decode length per unit which is 2+1(sos))
        ...
        if self.fix_kp_num_len:
            ...
            # max_kp_num is number of units or N = 20, kp_len = 2
            batch_size, max_kp_num, kp_len = tokens.size()
            max_tgt_len = max_kp_num * kp_len
            # print('max_tgt_len: ',max_tgt_len, max_kp_num, kp_len)

            D = control_embed.size(-1)

            # print('\n Control embedding dimension: ', control_embed.shape)
            # original control embed dimension is 16, 20, 512
            control_embed = control_embed[0].unsqueeze(0).expand(beam_batch_size, max_kp_num, D).unsqueeze(-2)
            # print('\n New Control embedding dimension: ', control_embed.shape)
            ...
            if (not self.config["generate"]) and (not temp_fix):  # training
            ...
            if self.config["generate"] or temp_fix:
