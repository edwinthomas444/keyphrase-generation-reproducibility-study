...
for key in test_keys[1:]:
    test_src_path[key] = Path(
        '../dataset/cross_domain_sorted/word_{}_testing_context.txt'.format(key))
    test_trg_path[key] = Path(
        '../dataset/cross_domain_sorted/word_{}_testing_allkeywords.txt'.format(key))
...
train_save_path_big_unsup = Path(
    '../processed_data/kp20k_big_unsup/train.jsonl')
dev_save_path_big_unsup = {}
for key in dev_keys:
    dev_save_path_big_unsup[key] = Path(
        '../processed_data/kp20k_big_unsup/dev_{}.jsonl'.format(key))
test_save_path_big_unsup = {}
for key in test_keys:
    test_save_path_big_unsup[key] = Path(
        '../processed_data/kp20k_big_unsup/test_{}.jsonl'.format(key))
metadata_save_path_big_unsup = fspath(
    Path("../processed_data/kp20k_big_unsup/metadata.pkl"))

train_save_path_low_res = Path('../processed_data/kp20k_low_res/train.jsonl')
dev_save_path_low_res = {}
for key in dev_keys:
    dev_save_path_low_res[key] = Path(
        '../processed_data/kp20k_low_res/dev_{}.jsonl'.format(key))
test_save_path_low_res = {}
for key in test_keys:
    test_save_path_low_res[key] = Path(
        '../processed_data/kp20k_low_res/test_{}.jsonl'.format(key))
metadata_save_path_low_res = fspath(
    Path("../processed_data/kp20k_low_res/metadata.pkl"))
...
train_srcs, train_trgs, train_trgs_no_peos = process_data(
    train_src_path, train_trg_path, train=True, update_vocab=True)
...
for key in dev_keys:
    dev_srcs[key], dev_trgs[key], \
        dev_trgs_no_peos[key] = process_data(
            dev_src_path[key], dev_trg_path[key], train=False, update_vocab=True)
...
def vectorize_data(srcs, trgs, trgs_no_peos):
    ...
    trgs_no_peos_vec = [text_vectorize(trg_no_peos)
                        for trg_no_peos in trgs_no_peos]
    ptr_srcs_vec, oov_nums = create_ptr_src(srcs, srcs_vec)
    labels = [create_labels(trg, trg_vec, src) for trg_vec, src, trg in zip(
        trgs_no_peos_vec, srcs, trgs_no_peos)]
...
dev_data = {}
for key in dev_keys:
    dev_data[key] = vectorize_data(
        dev_srcs[key], dev_trgs[key], dev_trgs_no_peos[key])
    jsonl_save(filepath=dev_save_path_big_unsup[key],
               data_dict=dev_data[key])

test_data = {}
for key in test_keys:
    test_data[key] = vectorize_data(
        test_srcs[key], test_trgs[key], test_trgs_no_peos[key])
