...
train_dataset = "openkp"
train_dataset = "kptimes"
train_dataset = "stackexchange"
dev_keys = [train_dataset]
test_keys = [train_dataset, "krapivin", "inspec", "semeval", "nus"]

train_src_path = Path(f'../dataset/{train_dataset}/train_src.txt')
train_trg_path = Path(f'../dataset/{train_dataset}/train_trg.txt')

dev_src_path = {}
dev_trg_path = {}
dev_src_path[train_dataset] = Path(f'../dataset/{train_dataset}/valid_src.txt')
dev_trg_path[train_dataset] = Path(f'../dataset/{train_dataset}/valid_trg.txt')

test_src_path = {}
test_trg_path = {}
test_src_path[train_dataset] = Path(f'../dataset/{train_dataset}/test_src.txt')
test_trg_path[train_dataset] = Path(f'../dataset/{train_dataset}/test_trg.txt')
for key in test_keys[1:]:
    test_src_path[key] = Path('../dataset/{}/test_src.txt'.format(key))
    test_trg_path[key] = Path('../dataset/{}/test_trg.txt'.format(key))

Path(f'../processed_data/{train_dataset}/').mkdir(parents=True, exist_ok=True)

train_save_path = Path(f'../processed_data/{train_dataset}/train.jsonl')
dev_save_path = {}
for key in dev_keys:
    dev_save_path[key] = Path(f'../processed_data/{train_dataset}/dev_{key}.jsonl')
test_save_path = {}
for key in test_keys:
    test_save_path[key] = Path(f'../processed_data/{train_dataset}/test_{key}.jsonl')
metadata_save_path = fspath(Path(f"../processed_data/{train_dataset}/metadata.pkl"))
...
def process_data(src_filename, trg_filename, train=True, update_vocab=True):
    ...
    for src, trg, trg_no_peos in zip(srcs, trgs, trgs_no_peos):
        ...
        if flag == 0:
            # print('len of src string: ', len(src))
            srcs_.append(src)
            trgs_.append(trg)
