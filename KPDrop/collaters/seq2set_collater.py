...
class seq2set_collater:
    ...
    def pad(self, items, PAD):
        ...
        # print('max len from collator: ', max_len)
    ...
    def keyphrase_dropout_fn(self, src, trg):
        ...
        for tokenized_kp in tokenized_kps:
            stemmed_kp = " ".join([self.stemmer.stem(kw.lower().strip())
                                   for kw in copy.deepcopy(tokenized_kp)])
            stemmed_kps.append(stemmed_kp)

        stemmed_src = " ".join([self.stemmer.stem(token)
                                for token in copy.deepcopy(src)])
        ...
        original_absent_keyphrases = [
            kp for kp in kps if kp not in original_present_keyphrases]
        ...
        new_present_keyphrases = [
            kp for kp in copy_present if kp not in new_absent_keyphrases]
        ...
        keyphrases = new_present_keyphrases + \
            ["<peos>"] + new_absent_keyphrases + original_absent_keyphrases
        keyphrases_no_peos = new_present_keyphrases + \
            new_absent_keyphrases + original_absent_keyphrases
        ...
        ptr_src_vec = [self.vocab2idx.get(token, src_token_dict.get(
            token, self.UNK_id)) for token in new_src]

        return new_src, new_trg_no_peos, src_vec, set_trg_vec, ptr_src_vec, set_label, len(src_token_dict)

    def collate_fn(self, batch):
        '''
        The data loader uses sampler given indices and a given batch size to sample from the 
        dataset class. the sampler just gives ordering of indices to be used in __getitem__ of dataset class
        the data loader then prepares a batch for it and then calls the collate_fn
        this function then is used to convert the batch values to tensors 
        and even sub-batching padding etc can be done if custom function is used.
        '''
        ...
        # print('original batch size: ',original_batch_size)
        ...
        while i < bucket_size:
            ...
            j = copy.deepcopy(i)
            # print('j: ',j,'i+inr: ',i+inr)

            while j < i + inr:
                # padding each batch of 12 line inputs to max length within the batch of 12
                srcs_vec_, src_masks = self.pad(
                    srcs_vec[j:j + inr_], PAD=self.PAD)
                ptr_srcs_vec_ = self.pad_no_mask(
                    ptr_srcs_vec[j:j + inr_], PAD=self.PAD)
        ...
        # print('meta batch length: ',len(meta_batches), len(meta_batches[0][0]['src']), meta_batches[0][0]['src_vec'].shape)
        batches = []
