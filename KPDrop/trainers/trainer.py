    ...
    def run(self, epoch, DataLoader, train, current_iter=None):
        ...
        with tqdm(total=self.generator_len, desc=self.desc, position=0, disable=self.no_display) as pbar:
            ...
            for batches in DataLoader:
                # print('from dataloader for loop, shape of batch: ',len(batches))
                # print('data loader length: ', len(DataLoader))
                for batch in batches: # single batch with key values, values being tensor of bs, ..
                    # print('\nbatch shape from dataloader: ',batch['src_vec'].shape)
                    item = self.agent.run(batch, train=train)
                    if train:
                        loss = item["loss"]
                        accu_step = self.total_batch_size//batch["batch_size"]
                        # (1.)loss backward propogation after every batch (there is only one accumulation step in these model trainings)
                        self.agent.backward(loss / accu_step)
                    cumsum_batch_size += batch["batch_size"]
                    item = {k: v for k, v in item.items() if k != "loss"}
                    tracked_items.append(item)

                    # for every batch of data do below
                    if (cumsum_batch_size // self.total_batch_size == i+1) or (cumsum_batch_size == self.sample_len):
                        ...
                        if train:
                            ...
                            # (2.)agent.step() steps the optimizer and clips the gradient using gradient norm
                            # but does not step the scheduler
                            self.agent.step()
                            self.global_step += 1

                            # print('\n current step learning rate: ',self.agent.optimizer.param_groups[-1]['lr'])
