...
#### added as flag error due to bert-tokenization error
import sys
...
def process(json_file, dataset, output_path, divide_stem, tokenizer):
    print(json_file)
    total_count = 0
    include_count = 0
    noise_count = 0
    ...
    with open("./data/" + dataset + "/" + json_file,"r") as f:
        for idx, line in tqdm(enumerate(f)):
            ...
            # save presnt key phrases without filter nesting..
            ...
            for span in content.split("@#^"):
                ...
                if "#tag_is_here#" in span:
                    first = True
                    # For each word in the keyphrase, divide it into subwords. If it is the first word in the keyphrase, it will be marked as BX... , otherwise it will be marked as IX....
            ...
            # If the absent phrase is empty, add none
            if len(absent_set) == 0:
                absent_set.add("none")
...
def main():
    ...
    # Initialize tokenizer, because token and label need to be aligned
    # Therefore tokenize is required before entering BERT
    # tokenizer = tokenization.FullTokenizer(
    #     vocab_file="/home/thomased/work/codebase/coopsummer2023/UniKP/UniKeyphrase/vocabulary/bert-base-cased/vocab.txt",   
    #     do_lower_case=True)
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)


    if args.data_path == "all":
        # dataset_names = ['inspec', 'nus', 'semeval', 'kp20k', 'krapivin']
        dataset_names = ['kptimes','openkp', 'stackexchange']
        print("process all file")
        for dataset in dataset_names:
            print("dataset" , dataset)
            for json_file in os.listdir("./data/" + dataset):
                if "meng17token" in json_file:
                    print("json file: ",json_file)
                    process(json_file, dataset, args.output_path, args.divide_stem, tokenizer)
    else:
