...
def main():
    ...
    if args.local_rank == -1 or args.no_cuda:
        ...
        # set local rank to current gpu rank
    ...
    if args.local_rank != -1:
        ...
        logger.info('Running Distributed Data Parallel')
        model = DDP(model,
    ...
    if args.fp16:
        try:
            ...
            from apex.optimizers import FusedAdam # (commented newer import version)
            # from apex.contrib.optimizers import FusedAdam # uses old version of APex
        except ImportError:
            raise ImportError(
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch,
            ...
            iter_bar = tqdm(
                ...
                disable=args.local_rank not in (-1, 0))  # only print the progress bar for rank 0 gpu.. 
            for step, batch in enumerate(iter_bar):
                ...
                if args.use_bwloss:
                    ...
                    # print("NS loss %f" % next_sentence_loss.item())
                    # loss = masked_lm_loss + seq_label_loss + next_sentence_loss + weight_bw * bag_of_word_loss
                    loss = masked_lm_loss + seq_label_loss + weight_bw * bag_of_word_loss
                    ...
                    loss = masked_lm_loss + seq_label_loss
