...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, label_ids=None, label_mask=None, \
        ...
        if self.use_bwloss:
            ...
            # print('\nPrediction words device: ', predict_words.get_device(), 'prediction scores masked device: ', prediction_scores_masked.get_device())
            temp = torch.cuda.FloatTensor(prediction_scores_masked.shape)
            # print('temp device: ',temp.get_device(), 'current device: ',predict_words.get_device())
        ...
        if pair_x is None or pair_y is None or pair_r is None or pair_pos_neg_mask is None or pair_loss_mask is None:
            if self.use_bwloss:
                # absent KP loss, present KP loss, BOW loss
                # below is returned in UniKP+BOW case
...
# for testing (it uses beam search for decoding)
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def forward(self,
        ...
        # token_type_ids has full length of seq_a+seq_b+ intermediate special tokens.. 
        output_length = output_shape[1]

        # get the bert embeddings for the context part of the sequence (without absent key phrases)
        ...
        # for each token embedding we get a distribution logit of size num labels here 9 (for detecting whether KP or not: extractive)
        ...
        prev_encoded_layers = None
        curr_ids = input_ids
        # tensor.new creates tensor of same type as self and same device but with given size and data to fill..
        mask_ids = input_ids.new(batch_size, 1).fill_(self.mask_word_id)
        # next_pos is start of the seq_b or length of seq_a
        ...
        while next_pos < output_length:
            ...
            # print('curr ids length: ', curr_ids.size(), 'next pos: ', next_pos, 'curr length: ', curr_length, 'start pos: ', next_pos - curr_length)
            ...
            if self.pos_shift:
                ...
                # first iteration start_pos will be zero.. 
                start_pos = next_pos - curr_length
                # input ids for the model is concat of seq_a + 1 [MASK] token
            ...
            # print('x input ids: ', x_input_ids.shape, 'curr_token_type_ids: ', curr_token_type_ids.shape)
            # 2nd parameter here is a list of (b, num_tokens, embedding size) where each element in outer list corresponds to a layer..
            ...
            # last hidden layer's, last tokens (which is last absent tokens embedding)
            ...
            # get prediction score for that absent vec
            ...
            # append the first predicted token to output ids and repeat loop
    ...
    def beam_search(self,
        ...
        while next_pos < output_length:
            ...
            # type casting backpointers to long or int64 to avoid index being float by division..
            step_back_ptrs.append(back_ptrs.type(torch.int64))
            ...
            def select_beam_items(x, ids):
                ...
                # print('x .type: ', x.dtype, x.shape, ids.shape, ids.dtype, ids)
                y = torch.gather(x, 1, ids.type(torch.int64))
