...
def Bootstrap(data_A, data_B, n, R):
    ...
    for x in tqdm(range(0, R),total=R):
        ...
        # print(delta, delta_orig, delta> 2*delta_orig, r, R)
...
def main():
    if len(sys.argv) < 4:
        ...
        sys.exit(1)
    filename_A = sys.argv[2] # 1
    filename_B = sys.argv[1] # 2
    alpha = sys.argv[3]
    name = sys.argv[4]
    
    print('filename_A: ', filename_A)
    print('filename_B: ', filename_B)
    ...
    data_A = list(map(float,data_A))    #[:1000]
    data_B = list(map(float,data_B))    #[:1000]
    ...
    if(name=="Permutation"):
        ...
        if (float(pval) <= float(alpha)):
            print("Test result is significant with p-value: {}\n".format(pval))
            return
        else:
            print("Test result is not significant with p-value: {}\n".format(pval))
            return
